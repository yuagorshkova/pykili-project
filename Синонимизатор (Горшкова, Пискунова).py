...
def main():
    ...
    for word in words:
        if morph.parse(word)[0].tag.POS in ['NOUN', 'ADJF', 'VERB']:
            ...
            if synonym != word_normal and morph.parse(word)[0].tag.POS == morph.parse(synonym)[0].tag.POS:
    ...
    for i in range(len(new_sentence)):
        if i == 0 or '.' in punct[i-1] or '!' in punct[i-1] or '?' in punct[i-1]:
            new_sentence[i] = new_sentence[i].capitalize()

    for i in range(len(new_sentence)):
        print (new_sentence[i], punct[i], sep = '', end = '')
